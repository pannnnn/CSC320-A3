...
def propagation_and_random_search(source_patches, target_patches,
    ...
    while(i < while_iters+1):
            ...
            i +=1
    # large_array_for_offset.reshape((row, column,while_iters,2))
    # for i in range(row):
    #     for j in range(column):
    #             cur_R = np.array([random.uniform(-1, 1), random.uniform(-1, 1)])
    #             large_array_for_offset[i,j,:] = w * alpha ** i * cur_R
        ...
        if odd_iteration:
            for i in range(0,row):
                for j in range(0,column):
                    ...
                    # array_for_random_offset = np.zeros((while_iters+1,2)).astype(int)
                    # k = 1
                    # while(k < while_iters+1):
                    #         cur_R = np.array([random.uniform(-1, 1), random.uniform(-1, 1)])
                    #         array_for_random_offset[k,:] = w * alpha ** k * cur_R
                    #         k +=1
